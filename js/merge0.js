...
function loadcoursesource(course)
{
    for (let cl = 0; cl < 3; cl += 2)
    {
        ...
        if (s0 == null || s0 == 'null' || s0.length < 10)
            s0 = source0[cl / 2];
...
function colorcol()
{
    ...
    for (let i = 0; i < tbl0.rows[0].cells.length; i++)
        tbl0.rows[0].cells[i].style.backgroundColor = 'grey';
    for (let i = 0; i < tbl1.rows[0].cells.length; i++)
        tbl1.rows[0].cells[i].style.backgroundColor = 'grey';
    if (matcol1[0] != null) {
...
function selma(c, c1, c2, r)
{
    ...
    {
        if (matcol1[1] == c1 && matcol2[1] == c2)
        {
            delete mapcol[matcol1[1]];
            matcol1.splice(1, 1);
            matcol2.splice(1, 1);
        }
        if (matcol1[0] == c1 && matcol2[0] == c2)
        {
            delete mapcol[matcol1[0]];
            matcol1 = [];
            matcol2 = [];
        }
    }
...
{
    let M = ma;
    let s = ['<table style=border-collapse:collapse border=1 cellpadding=4><tr><td>Choose</td><td style=white-space:nowrap >Column of Left</td><td style=white-space:nowrap >Column of Right</td><td>Strength</td></tr>'];
    for (let r = 0; r < M.length && r < 6; r++)
        s.push("<tr><td><input id=choice" + r + " type=checkbox onclick=selma(this," + M[r][0] + "," + M[r][1] + "," + r + ")></td><td style=white-space:nowrap >" + (M[r][0]) + ": " + b[0][M[r][0] - 1] + "</td><td style=white-space:nowrap >" + (M[r][1]) + ": " + g[0][M[r][1] - 1] + "</td><td>" + (100 * M[r][2]).toFixed(0) + "%</td></tr>");
    s.push("</table>");
    ...
    } else
    {
        let kk = 0;
        let s = null;
        while ((s = document.getElementById('choice' + kk)) != null && (ma[kk][0] != matcol1[0] || ma[kk][1] != matcol2[0]))
            kk++;
        if (kk < 6 && kk < ma.length)
            s.checked = true;
        kk = 0;
        while ((s = document.getElementById('choice' + kk)) != null && (ma[kk][0] != matcol1[1] || ma[kk][1] != matcol2[1]))
            kk++;
        if (kk < 6 && kk < ma.length)
            s.checked = true;
    }
    ...
    if (xy == null)
        return;
    let xys = xy.split(/ /);
    matcol1[0] = parseInt(xys[0]);
    matcol2[0] = parseInt(xys[1]);
    mapcol[matcol1[0]] = matcol2[0];
    ...
    {
        xys = xy.split(/ /);
        matcol1[1] = parseInt(xys[0]);
        matcol2[1] = parseInt(xys[1]);
        mapcol[matcol1[1]] = matcol2[1];
...
function match()
{
    ...
    for (let r = tbl2.rows.length - 1; r > 0; r--)
    {
        tbl2.deleteRow(r);
    }
    for (let r = 1; r < g.length; r++)
    {
        let rw = tbl2.insertRow(-1);
        rw.bgColor = 'white';
    ...
    for (let k =1; k < b.length; k++)
        if (!mb.includes(k))misb.push(k);
     
    m = 0;
    let mn = 1;
    let msg = "<" + "li>";
    msg += "Some mismatches were found. If a manual match is found, just click the matched pair.";
    msg += "<" + "table bgcoloe=white border=1 style=border-collapse:collapse align=center cellpadding=4><tr><td valign=top><div  style=background-color:blue;color:white>Left Records without Matching</div>";
    msg += "<" + "table>";
    let  missleft = false;
    let rw0 = 0, rw1 = 0;
    for (let m of misb)
    {
        ...
        for (let l in mapcol)
        {
             msg += '<td bgcolor=white onclick="pair(this,' + m + ',' + (rw0++) + ')" >&nbsp;' + b[m][l-1] + '</td>';
        }
        msg += '<' + '/tr>';
    }
    ...
    for (let m of misg)
    {
        ...
        for (let l in mapcol)
        {
             msg += '<td bgcolor=white onclick="pair1(this,' + m + ',' + (rw1++) + ')">&nbsp;' + g[m][mapcol[l]-1] + '</td>';
        }
        msg += '<' + '/tr>';
    }
     
    msg += "</table></td></tr></table><br>";
    if ( misg.length ==0 && misb.length == 0)
        msg = '';
    ...
    for (let j = gi; j < g[0].length; j++)
    {
        let cl = 'red';
        let lb = 'choose...';
        for (let kk in mapcol)
        {
            if (mapcol[kk] == j + 1)
            {
                lb = b[0][kk - 1];
        ...
        s += '<td align=right style="padding:0px 0px 0px 0px"><input style="margin:-1px -1px -1px -1px;border:0px;color:red;border:1px ' + cl + ' solid; color:' + cl + '" id="field' + j + '" list="existing" value="' + lb + '"</td>';
    }
    s += "</tr>";
    for (let i = 1; i < g.length; i++)
    {
        ...
        if (g[i] == null)
        {
            ...
            for (let j = 0; j < maxcol; j++)
            {
                ...
                s += (j == 0 && i >= b.length ? 'onclick=moveto(' + i + ',this.parentNode)' : '') + ' align=' + (true ? 'left' : 'right   width=85  ') + '><div style="' + (j < bi ? "" : "width:80px;overflow:hidden") + '"><nobr>' + (g[i][j] == null ? "" : g[i][j]) + '</nobr></div></td>';
            }
        s += '</tr>';
    }
...
function accept(i, tr)
{
    if (confirm('Move that line to fill out this blank line because of match?'))
    {
        ...
        {
            if (j == 0 && tr.cells[j].innerHTML.replace(/&nbsp;/g, '') != '')
                return;
...
function loadfile(i)
{
    ...
    reader.onload = function (e)
    {
        var textArea;
        if (ii == 0)
            textArea = document.getElementById("bbsource");
        else
            textArea = document.getElementById("googlesource");
